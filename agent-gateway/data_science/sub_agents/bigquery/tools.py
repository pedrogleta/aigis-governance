...
def get_bq_client():
    ...
    if bq_client is None:
        bq_client = bigquery.Client(project=get_env_var("BQ_COMPUTE_PROJECT_ID"))
    return bq_client
...
def get_bigquery_schema(
    dataset_id, data_project_id, client=None, compute_project_id=None
):
    ...
    for table_row in query_job.result():
        ...
        if table_obj.table_type == "VIEW":
            ...
            if (
                ...
                and table_obj.external_data_configuration.source_format == "ICEBERG"
            ):
...
**Think Step-by-Step:** Carefully consider the schema, question, guidelines, and best practices outlined above to generate the correct BigQuery SQL.
   ...
   """
    ...
    response = llm_client.models.generate_content(
        model=os.getenv("BASELINE_NL2SQL_MODEL") or "gemini-2.5-flash",
        contents=prompt,
